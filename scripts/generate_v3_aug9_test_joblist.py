#!/usr/bin/env python3
"""Emit the v3 aug9_car TEST joblist.

5K test envs across 10 aug9_car_v3 templates -> 500 envs/template.
Disjoint from the training set (which uses SEED_BASE 300M and writes to
.../v3/aug9_car). Test seeds start at 500M and write to .../v3/test/aug9_car.
Templates are rescaled aug9 layouts (factor 0.21 -> 0.49x0.49 m arena).
Object size range: 6-16 cm (full side length), matches the training set.
Output dir: /scratch/dm1487/datasets/car_envs/v3/test/aug9_car/

Run:
    python scripts/generate_v3_aug9_test_joblist.py > scripts/v3_aug9_test_joblist.txt
"""
...
# Budget
TOTAL_ENVS = 5_000
SEED_BASE = 500_000_000   # disjoint from v2 (100M), v3_feb (200M), v3_aug9 train (300M), feb test (400M)
SEED_STRIDE_PER_TEMPLATE = 100_000   # plenty of headroom — 500 envs/template
SEED_STRIDE_PER_CHUNK = 2_000        # > envs/chunk so seeds never collide
CHUNKS_PER_TEMPLATE = 5              # 10 templates × 5 chunks = 50 commands, 100 envs/chunk
    ...
    for t_idx, xml in enumerate(templates):
        ...
        for c_idx in range(CHUNKS_PER_TEMPLATE):
            ...
            cmd = (
                ...
                f"--num-envs {n_envs} "
                f"--output-dir {OUTPUT_ROOT}/test/aug9_car "
                f"--num-workers 1 "
                f"--start-seed {seed} "
