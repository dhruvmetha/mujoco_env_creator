#!/usr/bin/env python3
"""Emit the v3 feb_car joblist.

500K envs across 422 feb_car templates → ~1185 envs/template.
Object size range: 6-16 cm (full side length).
Output dir: /scratch/dm1487/datasets/car_envs/v3/feb_car/

Run:
    python scripts/generate_v3_feb_joblist.py > scripts/v3_feb_joblist.txt
"""
...
# Budget
TOTAL_ENVS = 500_000
SEED_BASE = 200_000_000   # disjoint from v2's 100M base
SEED_STRIDE_PER_TEMPLATE = 10_000   # plenty of headroom per template
SEED_STRIDE_PER_CHUNK = 200         # > envs/chunk so seeds never collide
CHUNKS_PER_TEMPLATE = 10            # 422 templates × 10 chunks = 4220 commands
                                    # 4220 / 60 shards = ~70 cmds/shard, xargs -P 32
                                    # → all 60×32 = 1920 cores saturated

# New object size range — 6 to 16 cm full side length (was 4-10 in v2)
OBJ_SIZE_MIN = 0.06
OBJ_SIZE_MAX = 0.16
...
def main():
    ...
    envs_per_template = TOTAL_ENVS // n_templates       # 500000 / 422 = 1184
    ...
    for t_idx, xml in enumerate(templates):
        ...
        for c_idx in range(CHUNKS_PER_TEMPLATE):
            ...
            cmd = (
                ...
                f"--num-envs {n_envs} "
                f"--output-dir {OUTPUT_ROOT}/feb_car "
                f"--num-workers 1 "
                f"--start-seed {seed} "
